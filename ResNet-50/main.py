    ...
    if istrain:
        ...
        for epoch in range(700):
            ...
            with torch.no_grad():
                ...
                torch.save(model.state_dict(), 'D:/PyCharmPythonProject/ResNet50/test5/model/' + 'dataset' + '_model_' + str(epoch + 1) + '.pt')
            scheduler.step()

    istest = 1
    if istest:
        model.load_state_dict(torch.load('D:/PyCharmPythonProject/ResNet50/test5/model/' + '\dataset' + '_model_' + '1' + '.pt'))
        model.eval()
        with torch.no_grad():
            # test
            total_correct = 0
